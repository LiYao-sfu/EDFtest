...
test_that("CvM P-value for uniform sample", {
  x = c(0.3103205, -0.1386720, -0.5988813,  0.9483934, -0.5213117,
        0.7555062, -0.6821612,  0.7957394, -0.7387161,  0.1910647)
  wsq = CvM.uniform(x)
  p_value = CvM.uniform.pvalue(w=wsq)$P

  expect_equal(p_value,0.43739209)
})
...
test_that("CvM P-value for normal sample", {
  ...
  wsq = CvM.normal(x)
  p_value = CvM.normal.pvalue(w=wsq)$P

  expect_equal(p_value,0.49021528)
})
...
test_that("CvM for gamma sample", {
  ...
  p_value = CvM.gamma.pvalue(w=wsq,shape=shape)$P

  expect_equal(p_value,0.88483667)
})
...
test_that("CvM for logistic sample", {
  ...
  wsq = CvM.logistic(x)
  p_value = CvM.logistic.pvalue(w=wsq)$P

  expect_equal(p_value,0.54949935)
})
...
test_that("CvM for laplace sample", {
  ...
  wsq = CvM.laplace(x)
  p_value = CvM.laplace.pvalue(w=wsq)$P

  expect_equal(p_value,0.046049247)
})
...
test_that("CvM for weibull sample", {
  ...
  wsq = CvM.weibull(x)
  p_value = CvM.weibull.pvalue(w=wsq)$P

  expect_equal(p_value,0.56603163)
})
...
test_that("CvM for exponential sample", {
  ...
  wsq = CvM.exp(x)
  p_value = CvM.exp.pvalue(w=wsq)$P

  expect_equal(p_value,0.1653965)
})
